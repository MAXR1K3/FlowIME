...
internal sealed class PackageFamilyNameResolver
{
    public string? TryResolve(uint processId)
    {
        ...
        {
            uint length = 0;
            var firstResult = Kernel32Native.GetPackageFamilyName(process, ref length, null);
            if (firstResult == Kernel32Native.AppModelErrorNoPackage)
            {
                return null;
            }
            ...
            var secondResult = Kernel32Native.GetPackageFamilyName(process, ref length, buffer);
