...
- 扫描 Steam 与 Epic 本地清单，在游戏库中统一管理每款游戏的聊天输入配置
- 支持从 EXE 手动添加游戏、本地封面或程序图标，以及每游戏独立快捷键
...
以下截图来自 Windows 11 上实际运行的 FlowIME 深色界面。

### 当前输入状态

首页集中显示当前应用、输入法状态、匹配依据和自动切换状态。

![FlowIME 首页实机界面](docs/images/flowime-home.png)

### 应用规则

规则页用于配置全局默认状态和每个应用的独立输入状态。

![FlowIME 应用规则实机界面](docs/images/flowime-rules.png)

### 游戏输入库

游戏页提供游戏保护、Steam/Epic 本地游戏扫描、程序图标或本地封面，以及逐游戏聊天快捷键配置。

![FlowIME 游戏输入库实机界面](docs/images/flowime-games.png)
...
FlowIME 可以在非输入状态下维持 US 键盘布局，减少输入法与游戏快捷键冲突；进入文本输入场景后，再按配置恢复输入状态。（正在持续开发）

游戏输入库会读取 Steam 与 Epic 的本地安装清单。扫描只用于发现游戏，不会猜测或覆盖聊天按键；新配置以 `Enter` 打开聊天、`Enter` 发送、`Esc` 取消作为可修改的默认值。无法可靠确定主程序时，FlowIME 会要求用户手动关联实际游戏 EXE，而不会把启动器或反作弊组件当作游戏本体。
